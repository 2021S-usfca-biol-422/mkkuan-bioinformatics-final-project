...
```{r snp-call-quality}
# this is to visualize the SNPs quality scores of all the samples in the SHanghai data
vcf_with_metadata %>%
...
vcf_with_metadata %>%
  ...
  ggplot(aes(x = lubridate::ymd(Collection_Date),
             y = n)) +
  geom_line() +
  labs(title = "COVID-19 Samples Collected in Shanghai from January to February 2020",
       x = "Date",
       y = "Number of Collections") +
  theme_few()
```

**Figure 2.** The frequency of samples collected in Shanghai from late January to early February.

```{r hospitalization-oxcovid19-data}
...
connect_oxcovid19() %>%
  ...
  geom_line() +
  labs(title = "COVID-19 Hospitalization in China from January to February 2020",
       x = "Date",
...
**Figure 3.** Hospitalization records in China from late January to early February. 
